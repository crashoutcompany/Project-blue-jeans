...
- TEST USER: E2E admin via `tests/e2e/.auth/admin.json` (`e2e-role=admin` cookie; `E2E_PLAYWRIGHT=1` swaps in stub Neon Auth). Role/data: admin bootstrap as platform-funded owner when no membership row exists; closet may be empty without `DATABASE_URL`.
