...
- **The app degrades gracefully with no secrets set**: all four routes (`/`, `/closet`, `/generator`, `/style-profile`) render, the closet is empty, and `GET /api/db/ping` returns `{"ok":false,...}` instead of crashing. Runtime errors only surface when you exercise a feature whose secret is missing (e.g. saving a garment, generating a lookbook).
