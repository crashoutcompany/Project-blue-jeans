...
- **Better Auth** (`BETTER_AUTH_SECRET`, optional `BETTER_AUTH_URL`, `AUTH_GOOGLE_ID`, `AUTH_GOOGLE_SECRET`) — self-hosted sessions and Google OAuth in the same Neon database. See `docs/better-auth-env.md` (includes CI `NEON_API_KEY` / `TEST_AUTH_SECRET` / `EXPOSE_TESTING_API`).
