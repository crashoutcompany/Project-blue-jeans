...
> **Domain expert:** "Routing + **Today** shell — home that matches the glossary; try-on and per-account isolation can follow."
...
- Generator implementation is still a chat lookbook — product decision (hybrid **Include**/**Avoid**, today-scoped, sheet, up to 3 options, approve → **Outfit**) is ahead of code.
- Implementation is still admin-gated shared closet — product decision (**per Wearer account**) is ahead of code.
- Closet UI still uses Outfits as a category chip — product decision (mode tabs) is ahead of code.
- Implementation still generates AI editorial heroes only — try-on + **Wearer photo** + soft prompt is ahead of code.
- Slice A in progress: Today shell + routing + landing rewrite shipped in app; Generator still full-page; Closet mode tabs / uniqueness / try-on still ahead.
...
- Build after Today shell + landing: Closet mode tabs + Outfit uniqueness; **Wearer photo** / try-on; per **Wearer account** isolation.
